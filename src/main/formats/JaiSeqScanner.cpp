...
private:
  ...
  virtual bool GetSampleInfo() override {
    ...
    for (uint32_t i = 0; i < c_dfCount; i++) {
      ...
      if (sampIdx >= samples.size())
              samples.resize(sampIdx + 1);
      */
  ...
  virtual bool LoadInstr() override {
    ...
    if (memcmp(buf, "Inst", sizeof(buf)) == 0) {
      ...
      for (uint32_t i = 0; i < numRegions; i++) {
        ...
        uint32_t sampleNum = GetShortBE(regionTableIdx);
        regionTableIdx += 0x02;
        regionTableIdx += 0x04; /* unknown */
        regionTableIdx += 0x04; /* frequency multiplier */

        VGMRgn *rgn = AddRgn(regionTableIdx, 0x18, sampleNum, keyLow, keyHigh);
...
private:
  ...
  virtual bool GetInstrPointers() override {
    ...
    wchar_t buf[64] = {};
    wsprintf(buf, L"Bank_%02d", index);
    name = buf;
